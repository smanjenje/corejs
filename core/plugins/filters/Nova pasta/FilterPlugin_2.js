// core/plugins/FilterPlugin.PRO.js
// Versão PRO simplificada: otimizada, compatível com CollPlugin + DocsIndexPlugin

module.exports = ({ app } = {}) => {
  if (!app) throw new Error("FilterPlugin precisa do objeto app");
  ...
  const matches = (doc, criteria) => {
    ...
    for (const [field, cond] of Object.entries(criteria)) {
      ...
      if (
        ...
        return false;
      if ("$nin" in cond && Array.isArray(cond.$nin) && cond.$nin.includes(val))
        return false;
      if (
        "$startsWith" in cond &&
